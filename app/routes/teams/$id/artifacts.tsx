...
import { getArtifactsByTeam, getArtifactsByTeamCount } from '~/services/artifact.server';
import { requireCookieAuth } from '~/services/authentication.server';
import { getTeam } from '~/services/teams.server';

export const loader: LoaderFunction = async ({ request, params }) => {
  await requireCookieAuth(request);
  const [team, items, count] = await Promise.all([getTeam(params.id as string), getArtifactsByTeam(params.id as string), getArtifactsByTeamCount(params.id as string)]);

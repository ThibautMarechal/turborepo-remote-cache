...
const UserActions = ({ resource: user }: { resource: User }) => {
  ...
  return (
    <div className="flex gap-1">
      <Link to={`/users/${user.username}`} prefetch="intent" title="Show user" className="btn btn-xs btn-square btn-outline">
        <MagnifyingGlassIcon className="w-4 h-4" />
      </Link>
      <HasRights predicate={(u) => isAdmin(u) && !user.isSuperAdmin}>
        <Link to={`/users/${user.username}/edit`} prefetch="intent" title="Edit user" className="btn btn-xs btn-square btn-outline">
          <PencilIcon className="w-4 h-4" />
        </Link>
        <Link to={`/users/${user.username}/change_password`} prefetch="intent" title="Change password" className="btn btn-xs btn-square btn-outline">
          <KeyIcon className="w-4 h-4" />
        </Link>
        <Form method="post">
          <button className={cn('btn btn-xs btn-square btn-outline', { loading: isDeleting })} title="Delete user">
